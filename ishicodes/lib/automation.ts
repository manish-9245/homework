...
export async function performPush(tokenData: any, dayData: any) {
    // If repo_owner and repo_name are provided, clone the target repo into a temp dir,
    // copy the day's solutions, commit and push using the provided credentials.
    ...
    // Fallback: operate on local repository path (assumed to be configured)
    try {
        const repoPath = path.join(process.cwd(), '..');
        const git = simpleGit(repoPath);
        const dayDir = path.join(repoPath, `day${dayData.day}`);
        if (!fs.existsSync(dayDir)) fs.mkdirSync(dayDir, { recursive: true });
